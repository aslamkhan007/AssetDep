...
using AssetDeprciation.Models.ReportModel.ComanpyActDepReportModels;
...
namespace AssetDeprciation.Controllers.ReportController.CompanyActDepreciationReportController
{
    public class MonthlyReportController : Controller
    {
        ...
        public ActionResult MonthlyReport()
        {
            ...
            return View("~/Views/Report/CompanyActDepreciationReport/MonthlyReport/MonthlyReport.cshtml", model);
        }

        public JsonResult getMonthlyReport(string AccYear)
        {
            using (SqlConnection con = DBConnection.getConnection())
            {
                List<MonthlyReportModel> list = new List<MonthlyReportModel>();
                string sql = "Select * from Jct_Asset_Dep_Company_Monthly_Dep_Record WHERE AccYear='" + AccYear + "' and Status='A'";
                SqlCommand cmd = new SqlCommand(sql, con);
                SqlDataReader read = cmd.ExecuteReader();
                if(read.HasRows)
                {
                    read.Close();
                    SqlCommand cmd2 = new SqlCommand("Jct_Asset_Dep_CompanyAct_MonthWise_Dep_Fetch", con);
                    cmd2.Parameters.AddWithValue("@AccYear", AccYear);
                ...
                {
                    read.Close();
                    SqlCommand cmd1 = new SqlCommand("Jct_Asset_Dep_CompanyAct_MonthWise_Dep_Cal", con);
                    ...
                    if(read1.HasRows)
                    {
                        while(read1.Read())
                        {
                            ...
                            model.SubGroupCode = read1["SubGroupCode"].ToString();
                            ...
                            model.TotalDepreciation = (float)Convert.ToDouble(read1["TotalDepreciation"].ToString());
                            list.Add(model);
                        }
                        read1.Close();
                        var jsonData = new
